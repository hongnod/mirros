 ...
 *slab_list:connect to the slab free list which in his size
 */
struct slab_header{
	unsigned long base;
	int size;
...
struct slab_header_table {
	...
	struct list_head table_list; 	/*conect page*/
	struct list_head free_header;	/*conect slab_header*/
...
void inline header_list_switch(struct slab_header *header,
		struct page *pg,struct list_head *list,int dir)
{
	if(dir){
...
static inline void init_slab_header(struct slab_header *header,int size,unsigned long base)
{
	header->base = base;
	header->size = size;
...
static struct slab_header *get_new_slab_header(void)
{
	...
	/*
	 *find if there are headers in free list freed by other slab
	 */
	...
	/*
	 *at this time we needn't decrease the usage of the page
	 *this action will keep the slab header page not free
	 */
...
static void *get_kernel_slab(int size,unsigned long flag)
{
	...
	pg->free_base = page_to_va(pg);
	/*
	 *the last page was used as SLAB memory,so we need sub the conter.
	 */
	pg->usage--;
...
void *kmalloc(int size,unsigned long flag)
{
	...
	if((flag != GFP_KERNEL) && (flag != GFP_DMA) 
			&& (flag != GFP_RES))
		return NULL;
...
static void __kfree_slice_slab(struct page *pg,unsigned long addr)
{
	...
	/*
	 *first we need to find the slab_header of this memory section.
	 */
